...
if(id != null){
    ...
    editor.start();
    // modified import data
    let mod_import_data = { ...msfb.raw_data }
    if( msfb.raw_data ) {
      let node_data = []
      // update the html DOM
      for( the_node_key in msfb.raw_data.drawflow.Home.data ) {
        ...
        // node id
        let node_id = the_node.id
        // the updated html for the node
        let updated_html = update_the_nodes_with_raw_data(node_name)
        // previous html of the node
        let prev_html = msfb.raw_data.drawflow.Home.data[the_node_key].html // get the previous node dom
        ...
        // update the outputs
        node_data.push({ node_name, node_id, prev_options, current_options })
      }
  ...
  function showpopup(e) {
   ...
   function closemodal(e) {
    ...
    if( jQuery('#msfb-formulation-builder').length > 0 ) {
      ...
      if( !this_el.attr('data-formula-name') ) {
        Swal.fire({
          ...
          console.log(ok)
          this_el.attr('data-formula-name',ok.value)
          jQuery.ajax({
            url: msfb.ajax_url,
            type: "POST",
            dataType: "json",
            data: {
              action: "create_the_q_flow",
              dataset: ok.value
            },
            success:function(resp){
              console.log(resp)
      ...
      let interval = setInterval( async () => {
        ...
        let formulation_name = this_el.attr('data-formula-name')  ? this_el.attr('data-formula-name') : 'Untitled q-flow'
